...
function frequentVisitors(){
	local visitsPerIP=$(cat "$logFile" | cut -d " " -f 1 | sort | uniq -c)
	echo "$visitsPerIP" | while read -r line; do
...
function suspiciousVisitors(){
	...
	local suspiciousIPs=$(grep -f "$iocFile" "$logFile" | cut -d ' ' -f 1 | sort | uniq -c)
	echo "Suspicious IPs:"
	echo "$suspiciousIPs"
}
